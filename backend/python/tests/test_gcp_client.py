...
class TestSignBlobKwargs:
    ...
    def test_refreshes_when_expired(self, logger: logging.Logger, mocker: Any) -> None:
...
class TestGenerateSignedUrl:
    def test_signs_via_iam(self, logger: logging.Logger, mocker: Any) -> None:
        credentials = _service_account_credentials(mocker)
        storage_client = _patch_adc(mocker, credentials)
        bucket = storage_client.return_value.bucket.return_value
        bucket.blob.return_value.generate_signed_url.return_value = (
            "https://signed.test/existing"
        )

        client = GCPStorageClient(logger, "f4k-bucket")
        url = client.generate_signed_url("notes/existing.png", expiration_hours=24)

        bucket.blob.assert_called_with("notes/existing.png")
        bucket.blob.return_value.generate_signed_url.assert_called_once_with(
            expiration=timedelta(hours=24),
            method="GET",
            service_account_email="uploader@f4k-test.iam.gserviceaccount.com",
            access_token="ya29.test-token",
        )
        assert url == "https://signed.test/existing"

    def test_forbidden_is_wrapped(self, logger: logging.Logger, mocker: Any) -> None:
        from google.api_core import exceptions as gcp_exceptions

        credentials = _service_account_credentials(mocker)
        storage_client = _patch_adc(mocker, credentials)
        blob = storage_client.return_value.bucket.return_value.blob.return_value
        blob.generate_signed_url.side_effect = gcp_exceptions.Forbidden("no")

        client = GCPStorageClient(logger, "f4k-bucket")
        with pytest.raises(GCSStorageError, match="permission denied"):
            client.generate_signed_url("notes/existing.png")
