"""
Developer script for manually testing the fetch_route_polyline utility with
mock locations. Prints the encoded polyline and distance to stdout.

Run from backend/python/:
    docker-compose exec backend pytest scripts/fetch_route_polyline_test.py -v -s
"""

from uuid import uuid4

import pytest

from app.models.location import Location

# Register the snapshot models so SQLAlchemy can resolve the RouteStop.snapshot
# / Route.snapshot relationship strings when it configures all mappers.
from app.models.route_snapshot import RouteSnapshot  # noqa: F401
from app.models.route_stop_snapshot import RouteStopSnapshot  # noqa: F401
from app.utilities.routes_utils import fetch_route_polyline


@pytest.mark.asyncio
async def test_fetch_route_polyline_with_return() -> None:
    """Test fetching polyline with return to warehouse."""

    # Create mock locations
    loc1 = Location(
        location_group_id=uuid4(),
        name="Test 1 Loc 1",
        contact_name="Test 1 Loc 1",
        delivery_type="Family",
        address="123 Test St",
        phone_primary="tel:+1-519-576-1201",
        longitude=-80.50,
        latitude=43.45,
        halal=True,
        dietary_restrictions="",
        num_children=10,
    )

    loc2 = Location(
        location_group_id=uuid4(),
        name="Test 1 Loc 2",
        contact_name="Test 1 Loc 2",
        delivery_type="Family",
        address="124 Test St",
        phone_primary="tel:+1-519-576-1202",
        longitude=-80.51,
        latitude=43.46,
        halal=True,
        dietary_restrictions="",
        num_children=10,
    )

    polyline, distance_km = await fetch_route_polyline(
        locations=[loc1, loc2],
        warehouse_lat=43.40,
        warehouse_lon=-80.46,
        ends_at_warehouse=True,
    )

    assert isinstance(polyline, str)
    assert distance_km > 0.0
    assert len(polyline) > 0
    print(f"Encoded Polyline: {polyline}")
