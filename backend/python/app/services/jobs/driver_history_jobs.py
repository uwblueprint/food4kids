...
is today or earlier) and not yet frozen, in a single transaction per route:

1. **Freeze.** Create the RouteSnapshot (locking in warehouse coords and the
   credited length) + RouteStopSnapshot rows that lock in what was delivered.
   The presence of the snapshot is the freeze signal — there is no separate
   flag column.

2. **Credit mileage.** If the route has a driver, post one AUTO entry to the
   driver_history ledger (km = route.length, drive_date = the group's date).
   Totals are aggregates over the ledger, so nothing is ever double-added —
   and a partial unique index (one AUTO entry per route) makes double-credit
...
so it is either fully frozen-and-credited or untouched, and one failing
...
async def _freeze_and_credit_route(
    ...
    """Freeze one route and credit its driver, atomically in a fresh session.

    Returns True if a mileage credit was posted. Raises on failure (nothing
    is committed in that case — the route stays due for the next run)."""
    logger = get_logger()
    async with maker() as session:
        ...
        # One commit: snapshot, stop snapshots, and credit land together or
        # not at all.
...
async def process_daily_driver_history() -> None:
    """Freeze all due, un-frozen routes and credit their drivers' mileage.
    ...
    try:
        # --------------------------------------------------------------
        # 1. Scan for due, un-frozen routes — plain values only (IDs and
        #    dates), since each route is processed in its own session.
        #    Includes past dates, not just today: routes missed by a
        #    downed or crashed run stay due until a run succeeds.
        # --------------------------------------------------------------
        async with async_session_maker_instance() as session:
            ...
            #    missing, freeze nothing — loudly. The routes stay due, so
            #    this self-heals on the first run after they're set.
            # ----------------------------------------------------------
            ...
            if (
                ...
                logger.error(
                    ...
                    f"{len(due)} due routes or credit their mileage. They "
                    f"remain due and will be processed on the next run "
                    f"after coordinates are set."
                )
                return
        ...
        # --------------------------------------------------------------
        # 3. Freeze + credit, one fresh session/transaction per route:
        #    either the snapshot, its stop snapshots, AND the mileage
        #    entry all commit together, or none do. A failing route is
        #    logged and skipped without poisoning the rest of the run.
        # --------------------------------------------------------------
        frozen_count = 0
        credited_count = 0
        failed_count = 0
        for route_id, drive_date in due:
            try:
                credited = await _freeze_and_credit_route(
        ...
        logger.info(
            f"Froze {frozen_count} routes ({credited_count} mileage credits "
            f"posted, {failed_count} failures)."
        )
